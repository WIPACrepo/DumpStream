...
DumperNextOptions = ['Dump', 'Pause', 'DumpOne', 'Inventory', 'ForceInventory']
BUNDLECOLS = ["bundleStatus_id", "localName", "idealName", "UUIDJade", "UUIDGlobus", "size", "status", "useCount", "checksum", "LooseFileDir"]
FULL_DIR_STATI = ['unclaimed', 'processing', 'LTArequest', 'filesdeleted', 'problem', 'deprecated', 'neverdelete']
FULL_DIR_SINGLE_STATI = ['unclaimed', 'filesdeleted', 'problem', 'deprecated', 'neverdelete']
SLOTBAD = -2
SLOTRESERVED = -1
SLOTUNK = 0
DEBUGDB = False
...
def polediskinfobyslot(estring):
    ...
    try:
        ...
        print('polediskinfobyslot:  invalid slot number (1-27)', estring)
        return ''
    if inum < 1 or inum > 27:
        print('polediskinfobyslot:  slot number out of range (1-27)', inum)
        return ''
    params = (unstring, )
    try:
        stuff = query_db_final(query, params)
...
# hassle to select info by slot--there are only 27 and
...
def setslot(estring):
    ...
    try:
        ...
        return 'FAILURE'
    if testint1 < 1 or testint1 > 27 or testint2 < 0:
        print('setslot wants the first argument 1..27, the second poledisk_id', estring)
        return 'FAILURE'
